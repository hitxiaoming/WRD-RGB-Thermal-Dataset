...
class FastRoadNetPlus(ConvNeXt):
    ...
    def __init__(self, 
        ...
        if not FastRoadNetPlus._config_logged:
            logger = MMLogger.get_current_instance()
            printer = logger.info if logger else print
            printer(f"\n{'='*40}")
            printer(f" [FastRoadNetPlus] Configuration")
            printer(f"{'='*40}")
            printer(f" Input Mode:          {self.input_mode.upper()}") # 打印模式
            if self.input_mode == 'rgb_ir':
                printer(" -> (Using RGB + IR Channels)")
            elif self.input_mode == 'rgb_rgb':
                printer(" -> (Using RGB + RGB Channels - RGB Only Control)")
            elif self.input_mode == 'ir_ir':
                printer(" -> (Using IR + IR Channels - IR Only Control)")
            
            printer(f" Fix Weights (Freeze): {fix_weights}")
            printer(f" IR -> RGB Weights:   {ir2rgb_weights}")
            printer(f" RGB -> IR Weights:   {rgb2ir_weights}")
            printer(f" Symmetric Mode:      {symmetric_interaction}")
            printer(f"{'='*40}\n")
            FastRoadNetPlus._config_logged = True
        ...
        for i, stage in enumerate(self.stages):
            ...
            if i in self.out_indices:
                ...
                if self.gap_before_final_norm:
                    ...
                    feat = torch.cat([norm_layer(gap_rgb), norm_layer(gap_ir)], dim=1)
                    outs.append(feat.flatten(1))
                else:
                    feat = torch.cat([norm_layer(x_rgb), norm_layer(x_ir)], dim=1)
                    outs.append(feat)
    ...
    def print_interaction_weights(self):
        ...
        logger.info(f"--- FastRoadNetPlus [{self.input_mode.upper()}] Weights {mode_str} {fix_str} ---")
